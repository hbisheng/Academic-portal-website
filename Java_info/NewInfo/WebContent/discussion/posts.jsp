...
%>
		...
		<tr> <td>帖序号</td><td>主题</td><td>发帖人</td><td>时间</td><td></td><td>&nbsp</td></tr><%
		...
		{ 
			...
			++cnt;
			%><b><tr><td><%=cnt%></td> <td> <a href=<%=post_url %>><%=post.topic%></a></td>
				
				<td><a href=<%=user_url%>><%=post.initiator%></a></td> <td><%=post.time %></td></b><%
			
			if(user != null)
			{
				if(post.initiator.equals(user.nickname))
				{		
					%><td><form action=<%=this_url%> method = "post"><input type ="hidden" name = "post_id" value ="<%=post.id%>"/>
					<input type ="submit" class="text-warning" value="删除我的帖子" name = "delete"/></form></td><%
				}
				else if(user.permit_admin>0)
				{
					%><td><form action=<%=this_url%> method = "post"><input type ="hidden" name = "post_id" value ="<%=post.id%>"/>
					<input type ="submit"class="text-warning" value="管理员删帖" name = "delete"/></form></td><%
				}
			}
				%></tr><%
		} 
