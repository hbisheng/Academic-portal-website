...
public class JdbcDao {
	...
	public User addUser(User user) //参数user需要保证validate = true  需要提前检查重名
	{
		try {
		    ...
		    System.out.println("In addU:addUser");
		    if(conn != null)   
		    	conn.close();
		    if(Statement != null)
		    	Statement.close();
	...
	public User getUser(User user)//依次按照以下参数寻找  id, account, nickname, 它们都是唯一的，如果要跳过就置0或置null  
	{
		...
		try {			
			...
			else if((user.account != null) && !(user.account.equals("")))
			{
				 rs = stmt.executeQuery("select * from user where account=\""+user.account+"\"");
			}
			else if((user.nickname != null)&& !(user.nickname.equals("")))
			{
	...
	public void delCourse(int course_id) {
		...
		List<Integer> li = getUserCourse(course_id);
		for(Integer uid:li)
		{
			delUserCourse(uid, course_id);
		}
	}
	...
	public void addMedia(Media m){
		try {			
			Connection conn = DriverManager.getConnection(URL + DATABASENAME,"root","");
			PreparedStatement Statement=conn.prepareStatement("insert into media VALUES(NULL,?,?,?,?)");
			Statement.setInt(1,m.type);
		    Statement.setString(2,m.url);
		    Statement.setString(3,m.name);
		    Statement.setString(4,m.content);
		    Statement.executeUpdate();
	...
	public List<Integer> getMediaIndex(int type)
	{
		List<Integer> list_c= new ArrayList<Integer>();
		try {			
			Connection conn = DriverManager.getConnection(URL + DATABASENAME,"root","");
